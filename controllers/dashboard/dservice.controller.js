...
app.get('/',adminAuth, async (req, res, next) => {
 ...
 var include=[]
  ...
  include= [{
    ...
    try {
      const servicesData = await PRODUCTS.findAll({
...
app.post('/status',adminAuth,async(req,res,next) => { 
    ...
    try{
       ...
       const userData = await PRODUCTS.findOne({
         where: {
           id: params.id }
       });
    ...
    if(params.status=="0")  status=1
       const updatedResponse = await PRODUCTS.update({
         status: status,
...
app.post('/add',adminAuth,async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      const users = await PRODUCTS.create({
        name: data.serviceName,
...
app.post('/update',adminAuth,async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const users = await PRODUCTS.update({
        name: data.serviceName,
        description: data.description,
        type: data.type,
        price: data.price,
